...
public class IntersectionAgent extends Agent {
     ...
     public class WatchDog extends OneShotBehaviour {
          ...
          public void action() {
               ...
               if (!lineEmpty(lineTurn)) block(9000000);
               intersectionAccessAllowed = false;
          } // WatchDog Class

          public boolean lineEmpty(int lineNumber) {
               boolean theResult = false;
               Iterator<IntersectionAgent.inRoadAgent> iterator = delayedElements.iterator();
               while (iterator.hasNext() && !theResult) {
                    IntersectionAgent.inRoadAgent element = iterator.next();
                    theResult = borderlineOutsideIntersection(element.getApproach(), element.getX(), element.getY())
                                   && lineApproachCompatible(lineNumber, element.getApproach());
               }
               return theResult;
          }
     }
